...
async def main():
    ...
    put_markdown("## 💬 Вітаємо у чаті!\n")

    msg_box = output()
    put_scrollable(msg_box, height=300, keep_bottom=True)

    nickname = await input("Ввійти в чат", required=True, placeholder="Ім'я",
                           validate=lambda n: "Такий нік вже використовується!" if n in online_users or n == '📢' else None)
    online_users.add(nickname)

    chat_msgs.append(('📢', f'`{nickname}` доєднався до чату!'))
    msg_box.append(put_markdown(f'📢 `{nickname}` доєднався до чату'))
